...
// [CERO-GAS 2026-07-19] _consumirLotesFIFO ELIMINADO — portado a wh._consumir_lotes_fefo (SQL 527).
...
function _autoCloseDayGuiasImpl() {
  // [CERO-GAS 2026-07-19] no-op: el autocierre vive en pg_cron (wh.autocerrar_guias_viejas).
  return { ok: true, cerradas: 0, nota: 'autocierre migrado a pg_cron' };
}
...
function _autocerrarGuiasInactivasImpl() {
  // [CERO-GAS 2026-07-19] no-op: el autocierre por inactividad vive en pg_cron (wh.autocerrar_guias_inactivas).
  return { ok: true, cerradas: 0, nota: 'autocierre migrado a pg_cron' };
}
...
// [CERO-GAS 2026-07-19] _cerrarGuiaImpl RETIRADO FÍSICAMENTE. El cierre de guías (stock +
// kardex + lotes FEFO + herencia a zona) vive en wh.cerrar_guia_idempotente (SQL 528/530).
// Este stub fail-closed queda para que cualquier caller zombi (trigger viejo / replay) NO
// toque la Hoja ni la sombra Supabase jamás.
function _cerrarGuiaImpl(idGuia, usuario, idSesion, opts) {
  Logger.log('[CERO-GAS] cerrarGuia legacy invocado y BLOQUEADO para ' + idGuia);
  return { ok: false, error: 'CERRAR_GUIA_LEGACY_RETIRADO (el cierre es 100% Supabase: wh.cerrar_guia_idempotente)' };
}
...
// [CERO-GAS 2026-07-19] _actualizarLote ELIMINADO — portado a wh._sync_lote_desde_detalle (SQL 530).
