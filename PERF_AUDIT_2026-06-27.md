...
- **G2 registrarUbicacion (GPS) → ✅ CONSTRUIDO+DESPLEGADO (INERTE), 2026-06-27.** Tabla `mos.dispositivos_ubicaciones` + write `registrar_ubicacion` + 2 reads admin + purga pg_cron (SQL 279). WH 2.13.363 / ME 2.8.95 / MOS 2.43.368 Supabase-first + fallback GAS. Flag `GPS_DIRECTO`. **NO activado**: requiere desactivar trigger GAS `verificarSinSenal` antes (else spam push). Doc: `ProyectoMOS/CUTOVER_G2_gps_cero_gas.md`.
- **G4 getAdminPinsCache (PINs admin offline) → ✅ CONSTRUIDO+DESPLEGADO (INERTE), 2026-06-27.** RPC `mos.admin_pins_cache` (SQL 280, authenticated NO anon, flag `ADMIN_PINS_DIRECTO`). WH 2.13.364 `sincronizarAdminCache` Supabase-first + fallback GAS. Corre solo al login (no es drain). Security-sensible (PINs plaintext); endurecido vs GAS abierto. NO auto-activado.
- **G3** notificarInicioSesionVendedor (push login) y **G7** detenerEscuchaAudio (stop audio espía) → **diferidos**: fire-and-forget, NO recurrentes (no drenan cuota), atados a los subsistemas push/espía que tienen su propia migración. (G6 ya hecho.)
- **Resumen drains recurrentes:** G1 (120s) ✅ ACTIVO · G2 (5min) ✅ INERTE-listo. Los dos drains reales resueltos.
- También pendiente (perf, no cero-GAS): QW2 escalonar arranque (limitar concurrencia), CD4 lazy guia_detalle_operacional.
